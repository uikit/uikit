...
(function (global, factory) {
          ...
          for (const unit of units) {
            ...
            if (el.textContent !== digits) {
              ...
              if (digits.length !== el.children.length) {
                html$2(el, digits.map(() => "<span></span>").join(""));
              }
    ...
    var LightboxPanel = {
      ...
      events: [
        ...
        },
        {
          name: "beforeitemshow",
          handler(e) {
            html$2($(this.selCaption, this.$el), purify.sanitize(this.getItem().caption || ""));
            html$2(
      ...
      methods: {
        ...
        setItem(item, content) {
          trigger(this.$el, "itemloaded", [this, html$2(this.getSlide(item), content)]);
        },
        getSlide(item) {
    ...
    function handleMutation(records) {
      ...
      for (const { addedNodes, removedNodes, target, attributeName } of records) {
        for (const node of addedNodes) {
          apply$1(node, connect);
        }
        for (const node of removedNodes) {
          apply$1(node, disconnect);
        }
    ...
    function install({ modal }) {
      ...
      modal.alert = function(message, options) {
        return openDialog(
          ({ i18n }) => `<div class="uk-modal-body">${isString(message) ? message : html$2(message)}</div> <div class="uk-modal-footer uk-text-right"> <button class="uk-button uk-button-primary uk-modal-close" type="button" autofocus>${i18n.ok}</button> </div>`,
          options
        );
      };
      modal.confirm = function(message, options) {
        return openDialog(
          ({ i18n }) => `<form> <div class="uk-modal-body">${isString(message) ? message : html$2(message)}</div> <div class="uk-modal-footer uk-text-right"> <button class="uk-button uk-button-default uk-modal-close" type="button">${i18n.cancel}</button> <button class="uk-button uk-button-primary" autofocus>${i18n.ok}</button> </div> </form>`,
          options,
          () => Promise.reject()
        );
      };
      modal.prompt = function(message, value, options) {
        const promise = openDialog(
          ({ i18n }) => `<form class="uk-form-stacked"> <div class="uk-modal-body"> <label>${isString(message) ? message : html$2(message)}</label> <input class="uk-input" autofocus> </div> <div class="uk-modal-footer uk-text-right"> <button class="uk-button uk-button-default uk-modal-close" type="button">${i18n.cancel}</button> <button class="uk-button uk-button-primary">${i18n.ok}</button> </div> </form>`,
          options,
          () => null,
          () => input.value
        );
